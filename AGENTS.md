...
- OCR: OpenRouter + `x-ai/grok-4.6`. Cloud Agent secret adı: `OPENROUTER_API_KEY` (start script `apps/api/.env` yazar). Anahtar yoksa job `FAILED` + manuel giriş; `REDIS_URL` yoksa süreç içi kuyruk; `S3_ENDPOINT` yoksa `uploads/`.
